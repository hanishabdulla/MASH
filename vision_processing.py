...
# Connect to Arduino
serialInst = serial.Serial('/dev/tty.usbmodem1101', 9600)  # Adjust port name as needed
...
def detect_text(frame, keywords):
    """Detects text in the frame and checks for specified keywords."""
    ...
    while True:
        if serialInst.in_waiting:
            ...
            if "OMG" in packet_str:
                # Start a new thread to handle the capture and processing
                threading.Thread(target=capture_and_process).start()

def main():
    # Start a thread to listen to Arduino
