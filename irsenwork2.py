...
def process_image(image):
    global text_detected, last_detection_time
...
def capture_and_process(frame):
    ...
    if not text_detected or (current_time - last_detection_time >= 7):
        text_detected = False  # Reset the flag to allow new detection
...
def main():
    ...
    while True:
        ...
        # Display a countdown timer after text detection
        if text_detected:
            current_time = time.time()
            elapsed_time = current_time - last_detection_time
            if elapsed_time < 7:
                remaining_time = 7 - elapsed_time
                print(f"Next detection in: {remaining_time:.1f} seconds", end='\r')
            else:
                print("Ready for next detection    ", end='\r')
